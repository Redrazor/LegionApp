...
> **Data sources:** Unit data and card images are sourced from [tabletopadmiral.com](https://tabletopadmiral.com), with structured stats (upgrade slots, surge conversions, keyword glossary, command cards, points history) merged from the open-source [Legion HQ](https://github.com/Electrynth/legion-hq-web) project (MIT). Huge thanks to both for maintaining such thorough Legion databases.
...
`scraper/scrape.ts` fetches the current unit list and Cloudinary card images from tabletopadmiral.com, merges structured stats / upgrade slots / keywords / command cards from Legion HQ's `cards.json` (matched by name), decodes faction/rank keys (`scraper/fkey-maps.ts`), and writes:

- `public/data/units.json` · `upgrades.json` · `commands.json` · `products.json` · `keywords.json`
- `public/images/units/<slug>.webp` — self-hosted card scans
