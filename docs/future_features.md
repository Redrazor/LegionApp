...
- ✅ **Card Sources reference** (`feature/card-sources-reference`, was open item 8 "legality catalogue") — the
  legality-badge premise was thin (only 9 errata-removed cards; 0 previews), so it was reframed at kickoff
  into a **card provenance/sourcing audit**: Reference → **Card Sources** lists every card with Name · Source ·
  Date · Validity (Valid = officially AMG-sourced, links to its Browse drawer; Unknown = legacy/image-pending),
  searchable + type-filterable. Backed by a new `public/data/card-sources.json` (`npm run card-sources`) built
  from `amg-card-map.json` + owner-maintained `amg-doc-dates.json`. Doubles as the Feature 13 re-sourcing
  progress tracker. See **Feature 18** below.
...
## Feature 18 — Card Sources reference (provenance audit) — SHIPPED

**Status:** ✅ SHIPPED (`feature/card-sources-reference`). Owner-requested 2026-07-02; **reframed at kickoff**.

**Why reframed:** the original "is this card legal?" badge was thin — the catalogue's only per-card
*illegality* signal is **9 errata-removed upgrades** (0 unreleased units/upgrades/commands), so a legal/illegal
badge would be almost entirely green. The owner reframed it into a **card provenance / sourcing audit**: for
each card, *where was it sourced from, when, and is that official?* — which also doubles as a live tracker for
the Feature 13 AMG re-sourcing migration ([[feature-13-p7-cleanup]]).

**What shipped:**
- **Reference → Card Sources tab** (`src/components/reference/CardSourcesTable.vue`, wired into
  `ReferenceView.vue`): a searchable, type-filterable table of every catalogue card showing **Name · Source ·
  Date · Validity**. **Valid** = officially AMG-sourced (links to the card's Browse drawer); **Unknown** =
  legacy Legion HQ 2 scan or image-pending placeholder. Name search, card-type filter, "Unknown only" toggle,
  and a valid/unknown summary. Card-centric (no format/faction filter — that dimension is composition-based,
  not per-card).
- **Data:** new `public/data/card-sources.json`, generated by **`npm run card-sources`**
  (`scraper/buildCardSources.ts`) from `scraper/amg-card-map.json` (image source per card) + a new
  owner-maintained **`scraper/amg-doc-dates.json`** (doc → human label + month/year, since only 6 of the 51
  source-doc filenames embed a date). Same provenance categorisation as `npm run amg:origins` (which writes the
  dev-facing `card_list_origin.md`), factored into the pure, tested `scraper/cardSources.ts`.
- **Pure + tested:** `buildCardSources` (builder) + `utils/cardSources.ts` (filter/link/summary/date) with
  12 specs. `store/cardSources.ts` loads the JSON. Current coverage: **794 valid / 73 unknown** of 867 cards.
- **Regenerate** after a scrape / apply batch / catalogue change (run order: `card-sources` after `amg:apply`).
  The owner refines labels/dates in `amg-doc-dates.json`; blank dates render "—" and don't affect validity.

---

**Original design (obsolete — legality-badge framing, retained for reference):**
