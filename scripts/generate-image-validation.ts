...
import { AMG_SOURCES, pdfBasename } from '../scraper/amgNormalise.ts'

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..')
const MAP_PATH = join(ROOT, 'scraper', 'amg-card-map.json')

// Optional `--faction <f>` filter: show only that faction's cards (units/commands by
// their catalogue faction so extras + cross-PDF cards like the Ewok commands are caught;
// upgrades by their source PDF since upgrades carry no faction tag). No flag = all cards.
...
<header>
  ...
  <div class="meta">${total} proposed mappings${FACTION ? ` · <b>${esc(FACTION)}</b> only` : ''} · NEW = AMG print-and-play candidate · review per source PDF · regenerated by <code>npm run images:validate</code></div>
  <div class="controls">
