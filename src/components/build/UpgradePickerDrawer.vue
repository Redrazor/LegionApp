    ...
    <div class="fixed inset-0 z-[60] flex justify-end">
      ...
      <aside class="relative flex h-full w-full max-w-md flex-col overflow-hidden border-l border-lg-border bg-lg-surface shadow-2xl">
        <div class="flex items-center justify-between border-b border-lg-border px-4 py-3">
          <h2 class="font-display text-sm font-bold uppercase tracking-widest text-lg-gold">
            {{ slotLabel(slot) }} Upgrade
          </h2>
          <button class="grid h-8 w-8 place-items-center rounded-lg text-lg-muted hover:bg-white/8" @click="emit('close')">✕</button>
        </div>
        <div class="border-b border-lg-border p-3">
          <input
            ...
            class="w-full rounded-lg border border-lg-border bg-lg-dark px-3 py-2 text-sm text-lg-text placeholder:text-lg-muted/60 focus:border-lg-gold/60 focus:outline-none"
          />
        </div>
        <div class="flex-1 overflow-y-auto p-2">
          <button
            class="mb-1 flex w-full items-center justify-between rounded-lg border border-dashed border-lg-border p-2 text-left text-sm text-lg-muted hover:border-lg-gold/40"
            @click="emit('clear')"
          >Empty slot</button>
          <button
            v-for="u in candidates" :key="u.id"
            class="flex w-full items-center justify-between gap-3 rounded-lg p-2 text-left hover:bg-white/5 disabled:opacity-40"
            :disabled="u.isUnique && equippedIds.includes(u.id)"
            @click="emit('pick', u.id)"
          >
            <div class="min-w-0">
              <div class="flex items-center gap-1">
                <span v-if="u.isUnique" class="text-lg-gold text-xs">◈</span>
            ...
            <span class="flex-none font-display text-sm font-bold text-lg-gold">{{ u.cost ?? 0 }}</span>
          </button>
