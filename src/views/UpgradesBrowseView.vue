...
<template>
  <div>
    ...
    <div class="sticky top-[52px] z-30 -mx-4 mb-5 flex flex-wrap items-center gap-2 border-b border-lg-border bg-lg-bg/95 px-4 py-3 backdrop-blur-sm">
      ...
      />
      <select v-model="filters.slot" class="rounded-lg border border-lg-border bg-lg-surface px-3 py-2 text-base text-lg-text focus:border-lg-accent/60 focus:outline-none">
      ...
      <button v-if="filters.query || filters.slot || filters.character" class="rounded-lg border border-lg-border px-3 py-2 text-base text-lg-muted hover:text-lg-accent" @click="reset">Reset</button>
